...
public class Movement : MonoBehaviour {
	...
	private void Update() {
		if (rb != null) {
			...
			if (accelerate) { fwdspeed = Mathf.Min(fwdspeed + acceleration, maxSpeed); } else if (reverse) { fwdspeed = Mathf.Max(fwdspeed - acceleration, -maxSpeed); } else { fwdspeed = Mathf.MoveTowards(fwdspeed, 0, acceleration); }
			moveForce.z = fwdspeed;

			// Apply Steering with strleft / strright
			if (strleft) { strspeed = Mathf.Min(strspeed + rotateSpeed, maxSpeed); } else if (strright) { strspeed = Mathf.Max(strspeed - rotateSpeed, -maxSpeed); } else { strspeed = Mathf.MoveTowards(strspeed, 0, rotateSpeed); }
			rotateForce.y = strspeed;

			// Apply movement with Ascending / Descending
			if (ascend) { risespeed = Mathf.Min(risespeed + acceleration, maxSpeed / 2); } else if (descend) { risespeed = Mathf.Max(risespeed - acceleration, -maxSpeed / 2); } else { risespeed = Mathf.MoveTowards(risespeed, 0, acceleration); }
			moveForce.y = risespeed;

			// Apply Strifing with strfleft / strfright
			if (strfleft) { moveForce.x = -moveSpeed * 20; } else if (strfright) { moveForce.x = moveSpeed * 20; } else { moveForce.x = 0; }
		}
	}
	...
	public void HandleSteerRight(InputAction.CallbackContext context) {
		if (context.performed) { strleft = true; } else if (context.canceled) { strleft = false; }
	}
	public void HandleSteerLeft(InputAction.CallbackContext context) {
		if (context.performed) { strright = true; } else if (context.canceled) { strright = false; }
	}
