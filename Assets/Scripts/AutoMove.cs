...
public class AutoMove : MonoBehaviour {
	...
	private void Update() {
		...
		void CalculateMovement() {
			...
			if (accelerate && fwdspeed < maxSpeed) { fwdspeed += acceleration; moveForce.z = fwdspeed; } else if (!accelerate && fwdspeed > 0.0f) { fwdspeed -= acceleration; moveForce.z = fwdspeed; }
			if (reverse && fwdspeed > -maxSpeed) { fwdspeed -= acceleration; moveForce.z = fwdspeed; } else if (!reverse && fwdspeed < 0.0f) { fwdspeed += acceleration; moveForce.z = fwdspeed; }
			if (fwdspeed > -0.1 && fwdspeed < 0.1) { fwdspeed = moveForce.z = 0.0f; }

			if (ascend && risespeed < maxSpeed / 2) { risespeed += acceleration; moveForce.y = risespeed; } else if (!ascend && risespeed > 0.0f) { risespeed -= acceleration; moveForce.y = risespeed; }
			if (descend && risespeed > -maxSpeed / 2) { risespeed -= acceleration; moveForce.y = risespeed; } else if (!descend && risespeed < 0.0f) { risespeed += acceleration; moveForce.y = risespeed; }
	...
	public void Descend(bool startstop) {
		if (startstop) { descend = true; } else if (!startstop) { descend = false; }
	}
	public void MoveRight(bool startstop, int multi = 1) {
		if (startstop && rotateForce.y < maxRotateSpeed) { rotateForce.y += rotateSpeed * multi; } else if (!startstop) { rotateForce.y = 0; }
	}
	public void MoveLeft(bool startstop, int multi = 1) {
		if (startstop && rotateForce.y > -maxRotateSpeed) { rotateForce.y -= rotateSpeed * multi; } else if (!startstop) { rotateForce.y = 0; }
	}
	public void StrifeRight(bool startstop) {
		if (startstop) { moveForce.x = maxMoveSpeed * 4; } else if (!startstop) { moveForce.x = 0; }
	}
	public void StrifeLeft(bool startstop) {
		if (startstop) { moveForce.x = -maxMoveSpeed * 4; } else if (!startstop) { moveForce.x = 0; }
	}
	public void MoveForward(bool startstop) {
		if (startstop) { accelerate = true; } else if (!startstop) { accelerate = false; }
	}
